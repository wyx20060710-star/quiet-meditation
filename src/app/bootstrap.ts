...
import { GentleChime } from '../infrastructure/sound';

export async function bootstrap(): Promise<AppController> {
  try {
    const repository = await IndexedDbRepository.open();
    const controller = new AppController(repository, undefined, true, new GentleChime(), new BrowserChannel());
    await controller.initialize();
    return controller;
  } catch {
    const controller = new AppController(new MemoryRepository(), undefined, false, new GentleChime(), new NoopChannel());
    await controller.initialize();
    return controller;
  }
}
